/*=============================================================================
 * Archeia/Liquidize - Steamworks
 * By Liquidize & Archeia - htp://anima.mintkit.lol / http://archeia.moe
 * Archeia_Steamworks.js
 * Version: 1.00
 *
 * Copyright (C) 2016 Dan "Liqudize" Deptula & Archeia
 *
 ...
 *
 * TLDR WHY PLUGIN IS CALLED "ARCHEIA_STEAMWORKS": I, Liquidize, was asked to create
 * this at the request of Archeia. I name my plugins with a prefix, that prefix is
 * the name of the person whom asked me to make it, or the game name. She gave moral
 * support though and therefore is credited ;-;.
 *
 *=============================================================================*/
/*:
 * @plugindesc Plugin Description <Archeia_Steamworks>
 * @author Liquidize & Archeia
 *
 ...
 *
 * @param Cloud Saving
 * @desc Maybe one day this will be a thing.
...
(function ($) {
    ...
    $.Param.debugMode = eval($.Parameters['Steamworks Debug']);
    $.Param.cloudSaving = eval($.Parameters['Cloud Saving']);

    var steamworks = require("./js/libs/steamworks");
    ...
    DataManager.processSteamworksItemNotetags = function(group) {
        ...
        var note2 = /<(?:ACHIEVEMENT CLEAR|achievement taken|achievement rekted|unachieved):[ ](.*),([-+]\d+)\/>/i;
    ...
    DataManager.processSteamworksClassNotetags = function(group) {
        ...
        var note2 = /<(?:ACHIEVEMENT CLEAR|achievement taken|achievement rekted|unachieved):[ ](.*),(LEARN|UNLEARN),(\d+)\/>/i;
        for (var n = 1; n < group.length; n++) {
            var obj = group[n];
            var notedata = obj.note.split(/[\r\n]+/);
    ...
    Game_System.prototype.getSteamUserObject = function() {
        if (SceneManager.steamworksInitialized()) {
            ...
            if ($.Param.debugMode) console.warn("[ARCHEIA STEAMWORKS] Steamworks is not Initialized.");
            return null;
        }
    };
    ...
    Game_System.prototype.getSteamName = function() {
        if (this.getCachedSteamUser()) {
            ...
            if ($.Param.debugMode) console.warn("[ARCHEIA STEAMWORKS] Unable to obtain Steam Name. Is Steamworks Initialized?");
            return null;
        }
    };
    ...
    Game_System.prototype.getSteamLevel = function() {
        if (this.getCachedSteamUser()) {
            ...
            if ($.Param.debugMode) console.warn("[ARCHEIA STEAMWORKS] Unable to obtain Steam Level. Is Steamworks Initialized?");
            return null;
        }
    };
    ...
    Game_System.prototype.getSteamAccountId = function() {
        if (this.getCachedSteamUser()) {
            ...
            if ($.Param.debugMode) console.warn("[ARCHEIA STEAMWORKS] Unable to obtain Account Id. Is Steamworks Initialized?");
            return null;
        }
    };
    ...
    Game_System.prototype.getSteamStaticAccountId = function() {
        if (this.getCachedSteamUser()) {
            ...
            if ($.Param.debugMode) console.warn("[ARCHEIA STEAMWORKS] Unable to obtain Static Account Id. Is Steamworks Initialized?");
            return null;
        }
    };
    ...
    Game_System.prototype.getCachedSteamUser = function() {
       if ($.CachedUser) {
           ...
           return $.CachedUser = this.getSteamUserObject();
       }
    };
    ...
    Game_System.prototype.saveTextToCloud = function(file,contents,callback,errorcallback) {
      if (SceneManager.steamworksInitialized()) {
    ...
    Game_System.prototype.readTextFromCloud = function(file,callback,errorcallback) {
        if (SceneManager.steamworksInitialized()) {
    ...
    Game_System.prototype.isCloudSyncEnabledForUser = function() {
        if (SceneManager.steamworksInitialized()) {
    ...
    Game_System.prototype.isCloudEnabled = function() {
        if (SceneManager.steamworksInitialized()) {
    ...
    Game_System.prototype.enableCloud = function(flag) {
        if (SceneManager.steamworksInitialized()) {
    ...
    Game_System.prototype.getCloudQuota = function(callback,errorcallback) {
        if (SceneManager.steamworksInitialized()) {
    ...
    //================================================================================
    // Cloud Saving - coming to a meme near you.
    //================================================================================
    ...
    Game_Interpreter.prototype.pluginCommand = function(command,args) {
      if (command.toLowerCase() == "steamworks") {
          ...
          switch (action) {
              ...
              case "getrekted": // I, Liquid, am a child at heart, this must be kept in in order for the children of the world to get their daily dose of memes.
                  if (args.length < 2) return; // no achievement name given.
                  args = args.splice(1,args.length - 1);
                  $gameSystem.clearAchievement(args.join(''),$gameSystem.onAchievementCleared,null);
                  break;
              case "devtest": // outputs the value of all returnable commands to console
                  for (var i = 0; i < 7; i++) {
                      console.warn($gameVariables.value(i));
                  }
                  break;
          }
      } else {
...
ArcheiaSteamworks = Archeia.Steamworks;
