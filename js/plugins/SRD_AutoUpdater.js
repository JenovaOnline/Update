...
_.downloadUpdateFile = function(url) {
	...
	_.download(url, _.localPath('save/updates.game'), function() {
		_.loadUpdateFile();
	})
};
...
_.loadUpdateFile = function() {
	var fs = require('fs');
	fs.readFile(_.localPath('save/updates.game'), 'utf8', function(err, contents) {
...
_.deleteUpdateFile = function() {
	var fs = require('fs');
	fs.writeFile(_.localPath('save/updates.game'), '', function(err) {
		if (err) throw err;
	})
};
...
_.localFilePathVersion = function() {
	return StorageManager.localFileDirectoryPath() + 'version.game';
};
...
Scene_AutoUpdate.prototype.setupParallax = function() {
	...
	this._parallax.bitmap = _.loadImage('UpdateBackground');
